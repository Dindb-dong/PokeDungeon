...
export function calculateAccuracy(baseAccuracy: number, ability: AbilityInfo[], accRank: number, dodgeRank: number): boolean {
  // 명중률 계산\
  let hitProb = 1;
  hitProb *= baseAccuracy / 100
  // TODO:명중률, 회피율 계산 필요 
...
export function calculateCritical(baseCritical: number, ability: AbilityInfo[], criRank: number, poison?: boolean): boolean {
  // 기술 기본 명중률, 포켓몬 특성, 랭크업 상태 
  let criProb = 0;
  let criRate;
  if (ability.some(v => v.name === '대운')) {
    criProb += 1; // 대운 적용
  }
